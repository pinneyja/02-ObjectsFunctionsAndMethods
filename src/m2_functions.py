...
def main():
    ...
    """
    print(hypotenuse(3, 4))  # should return 5.0, which it does
    print(colored_line("red", 8))
...
def hypotenuse(side1, side2):
    return m.sqrt((side1 * side1) + (side2 * side2))
...
def colored_line(color1, pen_thickness):
    window = rg.TurtleWindow()

    custom_turtle = rg.SimpleTurtle()
    custom_turtle.pen = rg.Pen(color1, 5)
    custom_turtle.pen_down()

    green_turtle = rg.SimpleTurtle()
    green_turtle.pen = rg.Pen("green", pen_thickness)
    green_turtle.pen_down()

    green_turtle.forward(100)
    custom_turtle.backward(100)
